...
def load_dataset(min_count, max_seq_len):
    ...
    en_vocab = Vocab(words=_tokens(0, pair_seq), min_count=min_count)
    fr_vocab = Vocab(words=_tokens(1, pair_seq), min_count=min_count)
    en_lang = Lang(words=en_vocab, reserved_tokens=(SOS, EOS, PAD), nil_token=UNK)
    fr_lang = Lang(words=fr_vocab, reserved_tokens=(SOS, EOS, PAD), nil_token=UNK)
    new_pairs = []
